...
def init_cond(ntraj, nclass , nquant):
    ...
    # R_I_v[i][k] -> k is for nclass and i is for ntraj
    R_I_v = np.zeros((ntraj,nclass),dtype=np.complex_)
    for i in range(ntraj):
        for k in range(nclass):
            R_I_v[i][k] =   0 + sig_x*np.random.normal()    #-15.0
            print("position is: ",sig_x*np.random.normal())
    # R_I_v = (-15.0 + sig_x*np.random.normal()) for k in range(nclass) for i in range(ntraj) # position vector
    # position R_I_v[0][0] = -15 + random
...
def cal_force(force_I_l_v, R_I_v ,dt , ntraj, nquant, nclass):
    # print("ground state: ", force_I_l_v[0][0][0])
    # print("excited state force: ", force_I_l_v[0][1][0])
    ...
    for k in range(nclass):
        for i in range(ntraj):
            ...
            for j in range(2): # range of quantum states
                # print(C_I_l,R_I_v,var_sigma_sqr)
                # print("by")
...
def mean_pos_And_var(rho_I_l_l, R_I_v ,ntraj , nquant ,nclass):
    ...
    for j in range(nquant):
        ...
        # print("hey total rho is",total_rho)
...
def derivative_coeff(R_I_v, C_I_l, P_I_v ,clas_P_I_v ,force_I_l_v ,ntraj, nquant, nclass):
    ...
    for i in range(ntraj):
        ...
        for j in range(nquant):
            ...
            # print(type(iota_hbar))
            # print(C_dot_I_l[i][j])
            # C_dot_I_l[i][j] = -iota_hbar.imag * Ei[j] * C_I_l[i][j] - a - c
...
def evolve_parameters(R_I_v, C_I_l, clas_P_I_v, P_I_v_dot, C_dot_I_l ,dt , ntraj, nquant, nclass):
    ...
    # x = C_I_l[0][0] * complex.conjugate(C_I_l[0][0]) + C_I_l[0][1]* complex.conjugate(C_I_l[0][1])
    # if(x < 0.9 or x > 1.01): print(x)
    #
    # print(x)
    ...
    for i in range(ntraj):
        for k in range(nclass):
            R_I_v[i][k] = R_I_v[i][k] + (clas_P_I_v[i][k] /mass[k] ) * dt     #should i evolve R_I_v using current momentum  or after evovling momentum fist?
...
def main():
    ...
    #storing position with respect to time
    ...
    for t in range(ntim): # time evolution
        # print("clas_momentum:",clas_P_I_v[0][0])
        # x[t] = clas_P_I_v[0][0]
        ...
        for i in range(ntraj):
            for j1 in range(nquant):
                for j2 in range(nquant):
                    # C_star_I_l = np.transpose(C_I_l)
                    # rho_I_l_l[i][j1][j2] = C_star_I_l[j1][i]*C_I_l[i][j2]         ## np.matmul(C_I_l[i][j1],C_I_l[i][j2])  => when Coeff are in form of array
        ...
        # print("mean pos", mean_pos[0][0])
        # x[t] = var_sigma_sqr[0][0]
        # y[t] = mean_pos[0][0]
        # print(mean_pos,end="rukoo")
        # print(var_sigma_sqr)
        ...
        # y[t] = P_I_v[0][0]
        # print("quantum momtm:", P_I_v[0][0])
        ####********* confirm pehle momentum derive krna hai ya pehle as it is likhna hai
        # Classical force
        P_I_v_dot = deriv_clas_momentum(ntraj, nquant, nclass,R_I_v, rho_I_l_l, force_I_l_v , P_I_v)
        # print("PIV_dot:",P_I_v_dot[0][0])

        # # calculate the classical momentum clas_P_I_v for current time step
        # # if (t != 0 ):
        # clas_P_I_v = calc_clas_momentum(P_I_v_dot,clas_P_I_v , dt , ntraj,nclass)
        ...
        # y[t] = C_I_l[0][0]
        # x[t] = C_I_l[0][1]
        # print(C_I_l[0][0]*complex.conjugate(C_I_l[0][0]) + C_I_l[0][1] * complex.conjugate(C_I_l[0][1]))
        # print("cil_dot:",C_dot_I_l[0][0])
        # x[t] = C_I_l[0][0]*complex.conjugate(C_I_l[0][0])
        # y[t] = C_I_l[0][0]*complex.conjugate(C_I_l[0][0]) + C_I_l[0][1] * complex.conjugate(C_I_l[0][1])
        # z = C_I_l[0][1] * complex.conjugate(C_I_l[0][1])
        ...
        #print(C_I_l,R_I_v,P_I_v,P_I_v_dot,C_dot_I_l,force_I_l_v)
        # print("hey")
        # for i in range(ntraj):
        Ei, grad_Ei, dij = pot(R_I_v[0,0])
        kin_E = (clas_P_I_v[i][0] ** 2) / (2 * mass[0])

        # y[t] = Ei[0] + Ei[1] #+ C_I_l[0,1]* complex.conjugate(C_I_l[0,1]) * Ei[1]
        # x[t] = kin_E
        ...
        # for i in range(ntraj): # plotting trajectory evolution with time
        #     x[t][i] = R_I_v[i, 0]
